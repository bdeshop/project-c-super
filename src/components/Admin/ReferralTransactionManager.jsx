...
const ReferralTransactionManager = ({ language }) => {
  const { transactions, loading, error, fetchReferralTransactions, updateTransactionStatus } = useReferral();
  ...
  const translations = {
    en: {
      title: "Referral Transaction Management",
      date: "Date",
      referrer: "Referrer",
      referee: "Referee",
      amount: "Amount",
      ...
      updateSuccess: "Transaction status updated successfully",
    ...
    bn: {
      ...
      updateSuccess: "লেনদেনের অবস্থা সফলভাবে আপডেট হয়েছে",
      updateError: "লেনদেনের অবস্থা আপডেট করতে ব্যর্থ",
      approve: "অনুমদন করুন",
      markPaid: "পরিশোধিত হিসাবে চিহ্নিত করুন",
  ...
  const handleUpdateTransactionStatus = async (transactionId, status) => {
    try {
      ...
      if (result.success) {
        ...
        alert(result.message || t.updateError);
      }
    } catch (err) {
          ...
          <table className="min-w-full bg-white rounded-lg overflow-hidden">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="py-2 px-4 text-left">{t.referrer}</th>
            ...
            <tbody>
              {transactions.map((transaction) => (
                <tr key={transaction._id} className="border-b border-gray-200">
                  ...
                  <td className="py-2 px-4">
                    {transaction.referrer?.username || "Unknown User"}
                  </td>
                  <td className="py-2 px-4">
                  ...
                  <td className="py-2 px-4">
                    {transaction.status === "pending" && (
                      ...
                      >
                        {updatingId === transaction._id ? t.updating : t.approve}
                      </button>
                    )}
                    {(transaction.status === "pending" || transaction.status === "approved") && (
                      <button
                        onClick={() => handleUpdateTransactionStatus(transaction._id, "paid")}
                        disabled={updatingId === transaction._id}
                        className="bg-green-500 text-white px-3 py-1 rounded text-sm disabled:opacity-50"
                      >
                        {updatingId === transaction._id ? t.updating : t.markPaid}
                      </button>
                    )}
                  </td>
                </tr>
...
export default ReferralTransactionManager;